...
OUTDIR="${OUTDIR:-${ROOT_DIR}/toy-runs/cdro_markov_precond_bestshot_v1}"
...
run_cfg "pre16_proxy_frontfreeze" \
  --score-lr 0.001 \
  --control-lr 0.0002 \
  --lambda-lr 0.02 \
  --lambda-init 0.1 \
  ...
  --budget-scale 20.0 \
  --budget-ema-decay 0.9 \
  --budget-max-ratio 4.0 \
  --budget-schedule frontload \
  --budget-frontload-power 2.0 \
  --budget-frontload-floor 0.2 \
  --warmup-epochs 12 \
...
wait
echo "[INFO] All best-shot shortlist jobs finished."
