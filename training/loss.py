...
class EDMLossWdro:
    ...
    def __call__(self, net, images, labels=None, augment_pipe=None, sigma_override=None, noise_override=None):
        if sigma_override is None:
            rnd_normal = torch.randn([images.shape[0], 1, 1, 1], device=images.device)
            sigma = (rnd_normal * self.P_std + self.P_mean).exp()
        else:
            sigma = torch.as_tensor(sigma_override, device=images.device, dtype=torch.float32)
            if sigma.ndim == 1:
                sigma = sigma.reshape(-1, 1, 1, 1)
            elif sigma.ndim == 0:
                sigma = sigma.reshape(1, 1, 1, 1).repeat(images.shape[0], 1, 1, 1)
            else:
                sigma = sigma.reshape(images.shape[0], 1, 1, 1)
        ...
        if noise_override is None:
            ...
            n = torch.as_tensor(noise_override, device=y.device, dtype=y.dtype)
            if n.shape != y.shape:
                raise ValueError(f"noise_override shape {tuple(n.shape)} must match image shape {tuple(y.shape)}")
...
class EDMLossAdv:
    ...
    def __call__(self, net, images, labels=None, augment_pipe=None, **_kwargs):
        rnd_normal = torch.randn([images.shape[0], 1, 1, 1], device=images.device)
        sigma = (rnd_normal * self.P_std + self.P_mean).exp()
        weight = (sigma ** 2 + self.sigma_data ** 2) / (sigma * self.sigma_data) ** 2
        y, augment_labels = augment_pipe(images) if augment_pipe is not None else (images, None)
